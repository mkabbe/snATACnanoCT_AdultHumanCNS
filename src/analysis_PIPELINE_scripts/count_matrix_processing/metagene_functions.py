...
def extractMarkerGenes(rna_adata, n_genes = 100, write_only=False):
    """
    ...
    if write_only is True, dictionary is not returned, but markers are written to files.
    """
    ...
    ctype_markers = marker_genes
        ...
        if not os.path.exists("data/celltype_markers"):
            os.mkdir("data/celltype_markers")
    
        for c in marker_dict.keys():
            with open("ref/gene_promoters.bed","r") as infile:
                with open(f"data/celltype_markers/{c}_markers.bed","w") as outfile:
                    for line in infile:
...
def imputeGA(gadata, t=3, knn=5, decay=1, solver="exact", knn_dist="cosine", n_jobs=1, 
             gene_list: Union[Literal['all_genes', 'pca_only'], Sequence[str], None] = None):
    """
    Run MAGIC to impute gene activity scores. Returns imputed GA object. 
    gadata must be a GA matrix.
    """
    
    magic_op = magic.MAGIC(t=t, 
                           knn=knn, 
                           decay=decay,
                           solver=solver, 
                           knn_dist=knn_dist, 
    ...
    ga_magic = magic_op.fit_transform(gadata, genes=gene_list)
    return ga_magic